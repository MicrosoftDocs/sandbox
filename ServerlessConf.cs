...
namespace TwitterFlic
{
    public static class ServerlessConf
    {
        private static TwitterContext _twitterCtx = null;
        private static IDictionary<string, string> _messageMap;

        [FunctionName("ServerlessConf")]
        public static async Task<HttpResponseMessage> Run([HttpTrigger(AuthorizationLevel.Function, "POST", Route = "notify/{messageType:alpha}")]HttpRequestMessage req, string messageType)
        {
            if (_twitterCtx == null)
            {
